...
def evaluate_model(inv_y, inv_yhat):
    ...
    print('MAPE:%.1f, MSE:%d, RMSE:%d' % (mape, mse, rmse))

    # with open('price/Taipei1_price.csv', 'a', encoding='utf8') as f:
    #     writer = csv.writer(f)
    #     writer.writerow(['%.3f' % mape, '%.3f' % mse, '%.3f' % rmse])

    pyplot.title('1-month Amount Predict for Fongshan')
...
def load_data(db, cursor):
    ...
    train_sql = """SELECT amount, price, d1_price, d1_origin_price, p_banana_d1_price, exchange_rate_price FROM Prediction_Source
    WHERE (market_no = '830') and (trade_date between '2012-01-01' and '2020-04-30');"""

    veri_sql = """SELECT amount, price, d1_price, d1_origin_price, p_banana_d1_price, exchange_rate_price FROM Prediction_Source
...
def train_model(train_X, train_y, test_X, test_y):
    ...
    # 當監測值 loss 不再改善時，如發現損失沒有下降，則經過 patience 次 epoch 後停止訓練。
    estop = EarlyStopping(monitor='loss', patience=100)

    # fit network
    history = model.fit(train_X, train_y, epochs=100, batch_size=10, validation_data=(test_X, test_y), verbose=2,
                        shuffle=False, callbacks=[estop])
    # # 將 epoch 的訓練結果保存在 csv 文件中
    # logger = CSVLogger('Multivariate_Time_Series_LSTM_Taipei1_price_0623-Copy1.log')

    # plot history
    loss = history.history['loss']
    val_loss = history.history['val_loss']
    pyplot.title('Loss')
    pyplot.plot(range(len(loss)), loss, label='loss(train)')
    pyplot.plot(range(len(val_loss)), val_loss, label='val_loss(test)')
    pyplot.legend(loc='best')
    pyplot.xlabel('epoch')
    pyplot.ylabel('loss')
    pyplot.show()
...
def main():
    ...
    # model.save('model/Taipei1_price.h5')
