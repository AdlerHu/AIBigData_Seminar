...
def load_data():
    ...
    sql = '''select price, amount, d1_price, d1_origin_price, p_banana_d1_price, exchange_rate_price
...
def split_dataset(data):
    ...
    train, test = data[:1975], data[1975:]

    print(len(train))
    print(len(test))

    # restructure into windows of monthly data
    train = array(split(train, len(train) / 25))
    test = array(split(test, len(test) / 25))
...
def to_supervised(train, n_input, n_out=25):
    # flatten data
...
def evaluate_forecasts(test_groups, actual, predicted):
    ...
    pred = predicted.reshape((test_groups, 25))
...
def predict_future(dataset, model, n_input):
    ...
    print('Predicted Price for 25-Day:')
    print(future[0])
